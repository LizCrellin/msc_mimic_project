...
for var in ['admission_age', 'los_icu', 'los_hosp']:
    rows.append({
        ...
        'los_7 = 0': f'{group0[var].median()}[{group0[var].quantile(0.25)}\u2013{group0[var].quantile(0.75)}]',
        'los_7 = 1': f'{group1[var].median():.2f}[{group1[var].quantile(0.25):.2f}\u2013{group1[var].quantile(0.75):.2f}]',
        'All': f'{icu_stays[var].median():.2f}[{icu_stays[var].quantile(0.25):.2f}\u2013{icu_stays[var].quantile(0.75):.2f}]'
    })
...
for var in ['gender', 'ethnicity', 'admission_type', 'admission_location', 'hospital_expire_flag']:
    for cat in sorted(icu_stays[var].unique(), key=str):
        rows.append({
            ...
            'All': f'{(icu_stays[var] == cat).sum()} ({(icu_stays[var] == cat).sum()/len(icu_stays) * 100:.2f}%)'
        })
